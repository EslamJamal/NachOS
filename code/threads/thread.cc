...
Thread::Thread (const char *threadName)
{
    ...
    stack = NULL;
    status = JUST_CREATED;
    threadID = 0;  // Just 0 for now
    finished = new Semaphore("thread finishing signal",0);
#ifdef USER_PROGRAM
    space = NULL;
...
void Thread::ThreadSemaphorePost(){
    finished->V();
}

void Thread::ThreadSemaphoreWait(){
    finished->P();
}

void Thread::SetStackMapping(int stackPos){

    StackMapping = stackPos;
}

int Thread::GetStackMapping(){
    return StackMapping;
}
//----------------------------------------------------------------------
// Thread::Fork
...
Thread::Fork (VoidFunctionPtr func, int arg)
{
    ...
    DEBUG('3',"Fork successfully passed");
...
Thread::Finish ()
{
    ...
    ThreadSemaphorePost();
    DEBUG('j',"posted on the semaphore\n");
